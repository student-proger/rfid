...
class rfidCard():
    # ID последней прочитанной карты. При записи происходит сверка текущей карты с последней прочитанной.
    lastReadID = []
    # Рабочий дамп данных
    dump = []
    # Дамп, в который помещаются данные последней прочитанной карты. Это необходимо для того, чтобы
    # сверять рабочий дамп с картой, а также иметь ключи для записи.
    cardDump = []
