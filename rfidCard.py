...
class rfidCard():
    ...
    dump = []
    # Временный дамп, в который помещаются данные для записи. Это необходимо для того, чтобы
    # в основной дамп поместить текущие данные с карты (для работы с ключами и битами доступа).
    writeDump = []
    ...
    def backupDump(self):
        """ Функция создаёт резервную копию текущего дампа для того,
        чтобы можно было прочитать записываемую карту перед записью на неё. """
        self.writeDump = self.dump[:]

    def restoreDump(self):
        """ Функция восстанавливает дамп из резервной копии после операции записи на карту. """
        self.dump = self.writeDump[:]
